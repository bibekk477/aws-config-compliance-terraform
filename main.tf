  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          StringEquals = {
            "s3:x-amz-acl" = "bucket-owner-full-control" #The PutObject action is allowed only if AWS Config sets the object ACL to "bucket-owner-full-control".
          }
        }
      },
      {
        ...
        Condition = {
          Bool = {
            "aws:SecureTransport" = "false" # Deny any S3 action on the config bucket (or its objects) if the request is not using HTTPS.
          }
        }
      }
    ]
